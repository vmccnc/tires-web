...
import {TireComponentProps,DetailedTire} from '../../interfaces/DetailedTire'
...
export const TireComponent: React.FC<TireComponentProps> = ({ id }) => {
  ...
  const tempUrls = new Array(5).fill(tire.url);
    ...
    <div className='tireWrapp'>
      <div className="tire">
          ...
          <div className='tire__block'>
            ...
            <div className="tire__thumbnails">
              {tempUrls.map((url, index) => (
                <img key={index} src={url} alt={tire.typeOfTire} className="tire__thumbnail" />
              ))}
            </div>
          </div>
        </div>
        <div className="tire__section tire__section--center">
          ...
          <button className="tire__add-to-cart-button">Добавить в корзину</button>
        </div>
      ...
      <div className='tireunder'>
        ...
        <div className='tireleft'>
          {[...Array(6)].map((_, index) => (
            <div className='tirebox' key={index}>
